...
void HalOledInit(int fds)
{
	...
	HalOledSendCmd(0x20);	//Set Memory Addressing Mode	
	HalOledSendCmd(0x10);	//0x02,Horizontal Addressing Mode;01,Vertical Addressing Mode;10,Page Addressing Mode (RESET);11,Invalid
	HalOledSendCmd(0xb0);	//Set Page Start Address for Page Addressing Mode,0-7
	HalOledSendCmd(0xc8);	//0xc0 Set COM Output Scan Direction 可换方向
	HalOledSendCmd(0x00); //---set low column address
	HalOledSendCmd(0x10); //---set high column address
	HalOledSendCmd(0x40); //--set start line address
	HalOledSendCmd(0x81); //--set contrast control register
	HalOledSendCmd(0xff); //亮度调节 0x00~0xff
	HalOledSendCmd(0xa1); //0xa0 --set segment re-map 0 to 127 可换方向
	HalOledSendCmd(0xa6); //--set normal display
	HalOledSendCmd(0xa8); //--set multiplex ratio(1 to 64)
	HalOledSendCmd(0x3F); //
	HalOledSendCmd(0xa4); //0xa4,Output follows RAM content;0xa5,Output ignores RAM content
	HalOledSendCmd(0xd3); //-set display offset
	HalOledSendCmd(0x00); //-not offset
	HalOledSendCmd(0xd5); //--set display clock divide ratio/oscillator frequency
	HalOledSendCmd(0xf0); //--set divide ratio
	HalOledSendCmd(0xd9); //--set pre-charge period
	HalOledSendCmd(0x22); //
	HalOledSendCmd(0xda); //--set com pins hardware configuration
	HalOledSendCmd(0x12);
	HalOledSendCmd(0xdb); //--set vcomh
	HalOledSendCmd(0x20); //0x20,0.77xVcc
	HalOledSendCmd(0x8d); //--set DC-DC enable
	HalOledSendCmd(0x14); //
	HalOledSendCmd(0xaf); //--turn on oled panel
	#endif
...
{  
	uint8 i,n;  
    HalOledSendCmd(0xA1); // Remap
	for(i=0;i<8;i++)for(n=0;n<128;n++)OLED_GRAM[n][i]=0;  
...
//列行扫描
    uint16 l_y,l_num,x0,y0,m,x,data_index=0,bit;
    l_num=ysize/8;
    if(ysize%8)
        l_num++;
    ...
    for(l_y=0; l_y<l_num; l_y++)
    {
        for(x=0; x<xszie; x++)
        {
            ...
            for(bit=0; bit<8; bit++)
            {
                HalOledDrawPoint(x0+x,y0+8*l_y+bit,(m&0x01));
                m>>=1;
            }
        }
    }
...
void        ss1306SetMirror(uint8 en)
{
      // the scan direction has been remapped in the oled design, so need reverse the setting here
        if(en == 0)
          ...
          en = 0 ;
        HalOledSendCmd(0xC0|(en<<3));                
        HalOledSendCmd(0xA0|en);        
}
...
void  get_cpu_ram_state(int *ram_used,int *cpu_used)
{
    ...
    *cpu_used = cal_cpuoccupy ((CPU_OCCUPY *)&cpu_stat1, (CPU_OCCUPY *)&cpu_stat2);
     ...
     *ram_used=(mem_stat.total-mem_stat.free)*100/mem_stat.total;
